...
export const excludePokemonKeys = [
  ...
  '0884-gmax',
  '0890-eternamax',
  '0892-single-strike-gmax',
  '0892-rapid-strike-gmax',
  '0970-mega',
  '0978-mega-curly',
  '0978-mega-droopy',
  '0978-mega-stretchy',
...
export const femalePokemonFrontPokedex = [
  '0003',
  '0012',
  '0019',
  '0020',
...
export const femalePokemonIconPokedex = [
  '0025',
  '0026',
  '0130',
  '0202',
...
export const femalePokemonOverworldPokedex = [
  '0003',
  '0025',
  '0154',
  '0202',
...
export const bigSizePokemonOverworldPokedex = [
  '0103',
  '0208',
  '0249',
  '0250',
...
export const MAP_MASTER_IDS: readonly string[] = [
  'p001',
  's000',
  's001',
  's002',
...
export class MasterData {
  ...
  loadJsonDataFromCache(
    ...
    try {
      ...
      console.log('Failed to load Master data from cache', error);
      throw error;
    }
  }

  loadItemData(data: any): void {
    Object.keys(this._itemData).forEach((k) => delete this._itemData[k]);
    Object.assign(this._itemData, data);
  }

  loadPokemonData(data: any): void {
    Object.keys(this._pokemonData).forEach((k) => delete this._pokemonData[k]);
    Object.keys(data).forEach((k) => {
      if (!excludePokemonKeys.includes(k)) {
        this._pokemonData[k] = data[k];
      }
    });
  }

  loadCostumeData(data: CostumeData): void {
  ...
  getPokemonData(pokedex: string): PokemonData | null {
    if (pokedex in this._pokemonData) {
      return this._pokemonData[pokedex];
    } else {
      return null;
    }
  }
  ...
  getCostumeData(key: 'skin'): string[];
  getCostumeData(gender: 'male' | 'female', category: 'outfits'): string[];
  getCostumeData(gender: 'male' | 'female', category: 'hairs'): string[];
  getCostumeData(gender: 'male' | 'female', category: 'hair_colors', hairId: string): string[];
  getCostumeData(
    key: 'skin' | 'male' | 'female',
    category?: 'outfits' | 'hairs' | 'hair_colors',
    hairId?: string,
  ): string[] {
    if (key === 'skin') {
      return this._costumeData.skin;
    }

    const genderData = this._costumeData[key] as CostumeGenderData;
    if (category === 'outfits') {
      return genderData.outfits;
    }

    if (category === 'hairs') {
      return genderData.hairs.map((hairArray) => hairArray[0]);
    }

    if (category === 'hair_colors' && hairId) {
      const hairArray = genderData.hairs.find((arr) => arr[0] === hairId);
      return hairArray ? hairArray.slice(1) : [];
    }
